...
main :: IO ()
main = do
    conn <- open "/home/gareth/projects/youduo/youduo-server/server-backup/database.sqlite3"
    ...
    FDB.withFoundationDB FDB.defaultOptions $ \db -> do
      forM_ (zip [0 :: Integer ..] videos) $ \(i, Video{..}) -> do
          FDB.runTransaction db $ do
              if Text.length videoSubtitles > 1024 * 10 then do
                liftIO $ putStrLn "Subtitles too big!"
              else do
                liftIO $ putStrLn $ "Inserting video " <> show i
                let key = T.encodeTupleElems ([T.Text videoId] :: [T.Elem])
                let value = T.encodeTupleElems ([T.Text videoTitle, T.Text videoSubtitles, T.Text videoFetchedAt] :: [T.Elem])
                FDB.set key value
    close conn